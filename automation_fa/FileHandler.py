...
def check_if_fail(file_path: str):
    try:
        with open(file_path, 'r') as file:
            content = file.read()
            if "signature: TestStatus.PASS" in content:
                return "Pass"
            elif "Terminate request has been received" in content:
                return "Terminate"
        return "Fail"
    except Exception as e:
        return "Unknown"
...
def unzip(zip_file, extract_folder, main_folder, zip_failed):
    try:
        ...
        failed_log_path = f"{main_folder}\\failed_zip.txt"
        if os.path.exists(failed_log_path):
            # Read the content of the file
            with open(failed_log_path, "r") as failed_log:
                failed_zips = failed_log.read().splitlines()

            # If zip_file is not already in the list, append it
            if zip_failed not in failed_zips:
                with open(failed_log_path, "a") as failed_log:
                    failed_log.write(f"{zip_failed}\n")
                print(f"Added {zip_file} to {failed_log_path}")
            else:
                print(f"{zip_file} is already logged in {failed_log_path}")
        else:
            # Create the file and write the zip_file name
            with open(failed_log_path, "w") as failed_log:
                failed_log.write(f"{zip_failed}\n")
            print(f"Created {failed_log_path} and added {zip_file}")
